...
- WebView 重新載入或 WebContent 終止後可重新附掛既有 PTY／SSH 工作階段，保留有界輸出緩衝、SFTP 綁定與未儲存的編輯器內容，並提供原生「重新整理工作區」復原入口。
...
- Recover from a WebView reload or WebContent termination by reattaching live PTY/SSH sessions, retaining bounded output, SFTP bindings and unsaved editor buffers, with a native Reload Workspace entry point.
